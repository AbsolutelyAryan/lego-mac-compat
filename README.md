...
The new C++/Objective-C++ renderer ABI is exercised with
`make -C native metal-renderer-probe`; this is the foundation for replacing
the shadow implementation incrementally without changing the legacy loader
ABI.
...
`LP32_METAL_BACKEND=1` is the forward-facing name for this experimental
backend boundary; it currently uses the same shadow implementation while the
guest shader, texture, and render-target translators are migrated. OpenGL
remains the fallback until a complete Metal frame is available.
